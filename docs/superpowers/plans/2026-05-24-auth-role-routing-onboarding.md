...
4. **Enable services** in the Console:
   ...
   - **Hosting:** (no action needed yet; Task 10 initializes it.)
...
7. **Super Admin seed (AFTER your first login, see Task 10 README):** in Firestore Console open `users/{your-uid}` and change `role` from `player` to `superadmin`.
...
```
pokerspot/
  firebase.json                         hosting config (Task 10)
  .firebaserc                           default project (Task 10)
  lib/
...
## Task 2: AppRole + AppUser model (TDD)
...
void main() {
  ...
  test('toMap round-trips', () {
    const u = AppUser(uid: 'x', phone: '+995555222222', displayName: 'Nino',
...
- [ ] **Step 4: Run, confirm PASS:** `flutter test test/features/auth/app_user_test.dart`

- [ ] **Step 5: Commit**
```bash
git add pokerspot/lib/features/auth/domain/app_user.dart pokerspot/test/features/auth/app_user_test.dart
git commit -m "feat: AppRole + AppUser domain model"
```

---

## Task 3: AuthRepository interface + FakeAuthRepository (TDD)
...
- [ ] **Step 3: Implement the interface** `lib/features/auth/domain/auth_repository.dart`:
```dart
...
- [ ] **Step 4: Implement the fake** `lib/features/auth/data/fake_auth_repository.dart`:
```dart
import 'dart:async';
...
- [ ] **Step 5: Run, confirm PASS:** `flutter test test/features/auth/fake_auth_repository_test.dart`

- [ ] **Step 6: Commit**
```bash
git add pokerspot/lib/features/auth/domain/auth_repository.dart pokerspot/lib/features/auth/data/fake_auth_repository.dart pokerspot/test/features/auth/fake_auth_repository_test.dart
git commit -m "feat: AuthRepository interface + fake (test phone codes)"
```

---

## Task 4: FirebaseAuthRepository (web phone OTP)

**Files:**
- Create: `pokerspot/lib/features/auth/data/firebase_auth_repository.dart`

Not unit-tested (it talks to live Firebase). The app uses the fake in tests; this runs at runtime in Chrome. Verified via `flutter analyze` + the live app.

- [ ] **Step 1: Implement** `lib/features/auth/data/firebase_auth_repository.dart`:
```dart
...
- [ ] **Step 2: Verify analyze clean:** `flutter analyze` → "No issues found!".

- [ ] **Step 3: Commit**
```bash
git add pokerspot/lib/features/auth/data/firebase_auth_repository.dart
git commit -m "feat: FirebaseAuthRepository (web phone OTP via signInWithPhoneNumber)"
```

---

## Task 5: UsersRepository interface + fake + Firebase impl (TDD)
...
## Task 7: Responsive wrapper + auth/onboarding l10n keys + Login screen (TDD widget test)
...
## Task 8: Onboarding screen (TDD widget test)
...
## Task 9: Auth redirect + role-based router + role home shells (TDD)
...
## Task 10: Firebase Hosting deploy + README
...
- **Spec coverage:** §6 onboarding (name+lang+GDPR) → Task 8; phone OTP auth → Tasks 3-4,7; role from `users` doc (phased per approved decision a) → Tasks 2,5,6,9; role-based shell → Task 9; i18n keys → Task 7; web-first + Hosting → Tasks 1,10. Custom claims + security rules deferred to Plan 7 (documented).
